...
ObjectFactoryBuilder::ObjectFactoryBuilder(IObjectFactory& factory)
: m_factory{factory}
{
}
...
{
    m_factory.registerToken(
        parser::TokenType::Boolean, [](const parser::Token& token) {
            return callback<typesystem::BooleanObject, bool>(token);
        });
    m_factory.registerToken(
        parser::TokenType::Label, [](const parser::Token& token) {
            return callback<typesystem::LabelObject, std::string>(token);
        });
    m_factory.registerToken(
        parser::TokenType::String, [](const parser::Token& token) {
            return callback<typesystem::StringObject, std::string>(token);
        });
    m_factory.registerToken(
        parser::TokenType::Integer, [](const parser::Token& token) {
            return callback<typesystem::NumberObject, int>(token);
        });
    m_factory.registerToken(
        parser::TokenType::Floating, [](const parser::Token& token) {
